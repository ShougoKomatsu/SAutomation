...
ReturnValue Flow_WriteImage(CString sDir, int iScene, CStringArray* saData)
{
	...
	ImgRGB* imgRGB=GetImgValuePointer(sDir, iScene, saData->GetAt(0));
	if(imgRGB == NULL){return RETURN_FAILED;}

	
	CString sFilePath;
	CString sArg;
	sArg.Format(_T("%s"), GetStrValue(sDir, iScene, saData->GetAt(1)));
...
/*
ReturnValue Flow_Threshold(CString sDir, int iScene, CStringArray* saData, CString* sReturnParam)
{
	...
	Object* obj=GetObjValuePointer(sDir, iScene, saData->GetAt(3));
